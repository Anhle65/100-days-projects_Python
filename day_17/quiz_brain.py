
class QuizBrain:
    def __init__(self, q_list):
        self.question_number = 0
        self.question_list = q_list
        self.score = 0

    def still_has_question(self):
        return self.question_number < len(self.question_list)

    def play_game(self):
        current = self.question_list[self.question_number]
        self.question_number += 1
        user_answer = input(f'Q{self.question_number}: {current.text} True/False ')
        correction = current.answer
        self.check_answer(user_answer, correction)

    def check_answer(self, user_answer, correction):
        if user_answer.lower() == correction.lower():
            self.score += 1
            print(f'Your score {self.score}/{self.question_number}')
        else:
            print("Your answer is false")
            print(f'The answer is {correction}')
            print(f'Your score {self.score}/{self.question_number}')

